...
let currentFacingMode = 'environment'; // start rear-facing
...
// ─── POPULATE CARD FROM mockData & state ────────────────────
function populateCameraCard() {
  const sessionName = mockData.workout?.session?.name || 'Training Day';
  document.getElementById('card-session-name').textContent = sessionName;

  const exercises = mockData.workout?.session?.exercises || [];
  
  const totalSets = exercises.reduce((a, b) => a + (b.sets || 0), 0);
  
  const totalVolume = exercises.reduce((acc, ex) => {
    return acc + ((ex.sets || 0) * (ex.load || 0));
  }, 0);

  const unit = state.settings?.units || 'kg';
  const streak = mockData.dashboard?.streak || 0;

  document.getElementById('card-sets').textContent = totalSets;
  document.getElementById('card-volume').textContent = totalVolume > 0
    ? totalVolume + unit
    : '—';
...
function drawStatsCardOnCanvas(ctx, W, H) {
  ...
  ctx.fillText('PULSE', innerX, y);
  y += 24;

  // Session name
  ctx.font = '700 20px system-ui';
  ctx.fillStyle = '#F0F0FF';
  ctx.fillText(document.getElementById('card-session-name').textContent, innerX, y);
  y += 28;
  ...
  const stats = [
    ...
    { value: document.getElementById('card-streak').textContent, label: 'Day Streak' }
  ];
  let statX = innerX;
  stats.forEach(function(stat) {
    ctx.font = '800 24px system-ui';
    ctx.fillStyle = '#F0F0FF';
    ctx.fillText(stat.value, statX, y);
    ctx.font = '500 11px system-ui';
    ctx.fillStyle = '#8888AA';
    ctx.fillText(stat.label.toUpperCase(), statX, y + 16);
    statX += 90;
  });
}
...
window.savePhoto = function() {
  ...
  a.download = 'pulse-progress-' + Date.now() + '.jpg';
  a.click();
}
...
window.sharePhoto = async function() {
  if (!capturedImageDataUrl) return;
  try {
    const blob = await (await fetch(capturedImageDataUrl)).blob();
    const file = new File([blob], 'pulse-progress.jpg', { type: 'image/jpeg' });
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], title: 'Pulse Progress' });
    } else {
      window.savePhoto(); // fallback
    }
  } catch (err) {
    window.savePhoto(); // fallback on any error
  }
}
...
window.retakePhoto = function() {
  ...
  document.getElementById('post-bar').classList.add('hidden');
}
